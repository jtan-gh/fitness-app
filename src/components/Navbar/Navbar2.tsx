  ...
  useEffect(() => {
    const updateNavigation = () => {
      if(logoRef && logoRef.current){
        const navItemsBounds = logoRef.current.getBoundingClientRect();
        const availableWidth = window.innerWidth - navItemsBounds.right;

        // Calculate the number of items to show in the primary menu
        const maxItemsInPrimaryMenu = calculateMaxItems(availableWidth);
        console.log("max items", maxItemsInPrimaryMenu)
  ...
  return (
    <div>
      <AppBar position="static">
        <Toolbar>
          <Typography ref={logoRef} variant="h1" style={{ whiteSpace: 'nowrap', marginRight: 'auto'}}>
            My Website
          </Typography>
